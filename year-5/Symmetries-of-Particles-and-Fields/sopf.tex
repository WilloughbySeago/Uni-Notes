...
\begin{document}
    ...
    \begin{dfn}{Lie Group}{}
        A \defineindex{Lie group}\footnote{pronounced \emph{lee}}, \(G\), is a continuous group for which the group multiplication has an analytic structure.
    ...
    \begin{dfn}{Lie Subalgebra and Ideals}{}
        ...
        An \defineindex{ideal} is a Lie subalgebra \(\lie{i}\) with the stronger condition that \(\commutator{g}{i} \in \lie{i}\) for all \(g \in \lie{g}\).
    \end{dfn}
    ...
    Similar to the group case a Lie algebra is \defineindex{simple!Lie algebra} if it has no nontrivial ideals, that is the only ideals are itself and the zero dimensional vector space, and it is not Abelian.
    ...
    \begin{dfn}{Adjoint Representation}{}
        The \defineindex{adjoint representation} of a Lie algebra, \(\lie{g}\), is the representation on \(\complex^{\dim \lie{g}}\) given by choosing the generators to satisfy
        \begin{equation}
    ...
    \begin{equation}
        \tensor{(T_a)}{^d_e} \tensor{(A_b)}{^e_c} - \tensor{(A_b)}{^d_e}\tensor{(A_a)}{^e_c} = i\tensor{(A_e)}{^d_c} \tensor{c}{^e_{ab}},
    \end{equation}

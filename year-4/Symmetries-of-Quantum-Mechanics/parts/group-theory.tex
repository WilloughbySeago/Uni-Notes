...
Many of theses hold for some relaxed version of a group, just check which of the group axioms are used in the proof.
...
\begin{dfn}{Morphism}{def:morphism}
    ...
    An \defineindex{isomorphism} between groups \(G\) and \(H\) is a bijective homomorphism.
...
\begin{exm}{}{}
    The permutations \(A_3 = \{\cycle{}, \cycle{1, 2, 3}, \cycle{3, 2, 1}\}\) form a normal subgroup of \(S_3\), this subgroup is called the alternating group and we will study it more in the future.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{The permutation \(\cycle{1, 2, 3, 4}\) acts on \((a, b, c, d)\), which can be done in steps where each step is a transposition.}
...
\begin{dfn}{Sign of a Permutation}{dfn:sign of a permutation}
    ...
    Now define \(\psi \colon S_n \to \{\pm 1\}\) by \(\psi(\sigma) = P(\sigma\action \vv{x})/\abs{P(\vv{\vv{x}})}\).
    Then if \(\psi(\sigma) = 1\) we say \(\sigma\) is an \defineindex{even permutation} and if \(\psi(\sigma) = -1\) we say that \(\sigma\) is an \defineindex{odd permutation}.
...
\begin{thm}{}{}
    ...
    \begin{proof}
        ...
        First notice that \(\psi(\sigma\rho) = P(\sigma\rho\action \vv{x})/\abs{P(\vv{x})}\).
        Now consider \(\psi(\sigma)\psi(\rho) = P(\sigma\action\vv{x})P(\rho\action\vv{x})/\abs{P(\vv{x})}^2\).
        Hence \(\psi(\sigma)\psi(\rho) = 1\)  if \(\sigma\) and \(\rho\) have the same parity and \(\psi(\sigma)\psi(\rho) = -1\) if \(\sigma\) and \(\rho\) have opposite parities.
        ...
        Therefore \(\psi(\sigma\rho) = P(\sigma\rho\action\vv{x})/\abs{P(\vv{x})}\) is 1 if \(\sigma\) and \(\rho\) are the same parity and \(-1\) if they are opposite parities.
        Hence \(\psi\) is a homomorphism.
        
        The kernel of \(\psi\) is \(A_n\), since even permutations map to \(1\) by definition.
...
These have 4, 6, 8, 12, and 20 faces, respectively, and are formed from equilateral triangles, squares, equilateral triangles, regula pentagons, and equilateral triangles, respectively.
...
For example, suppose there was an electric dipole and it aligned with the \(\integers_3\) symmetry axis.
...
\begin{figure}
    ...
    \caption{Methane is a tetragonal molecule, meaning that it has a central atom, here carbon, and four atoms arranged around it making the points of a tetrahedron.}
    \label{fig:methane}
\end{figure}

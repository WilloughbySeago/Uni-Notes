...
\begin{dfn}{Group}{dfn:group}
    Formally a \defineindex{group} is an ordered pair, \((G, \cdot)\), where \(G\) is a set and \(\cdot\) is a binary operation on \(G\) satisfying the following \define{group axioms}\index{group!axioms}:
    \begin{enumerate}
...
\begin{exm}{Permutation Group}{}
    The \define{permutation group}\index{permutation!group}\index{Sn@\(S_n\), permutation group}, \(S_n\), defined as the set of all permutations on \(n\) objects, is a group under function composition.
...
\begin{exm}{Cyclic Group}{}
    ...
    This is a group under multiplication, called the \define{cyclic group}\index{cyclic!group}\index{Zn@\(\integers_n\), cyclic group} of order \(n\).
...
\begin{dfn}{Group Size}{}
    Given some group \(G\) we classify it as \define{finite}\index{finite group}, \define{discrete}\index{discrete group}, or \define{continuous}\index{continuous group}, depending on whether \(G\) has a finite number of elements, the same number of elements as \(\integers\), or more elements than \(\integers\).
...
\begin{dfn}{Subgroup}{}
    ...
    \end{itemize}
    A subgroup is said to be a \define{proper subgroup}\index{subgroup!proper} if the subgroup is not equal to the full group, or the \define{trivial group}\index{trivial!group}, \(\{e\}\).
\end{dfn}
...
\begin{dfn}{Generators}{}
    ...
    We call \(g_i\) \define{generators}\index{generator}.
...
\begin{dfn}{Group Presentations}{}
    A \define{group presentation}\index{group!presentation} is a way of defining a specific group.
...
\begin{dfn}{}{}
    ...
    A \define{group action}\index{group!action} is a map, \(\varphi\colon G \times X \to X\), where we use the notation \(\varphi(g, x) = g\action x\).
...
\begin{dfn}{Direct Product}{dfn:direct product}
    ...
    Then we define a new group, \(H \times J\), called the \define{direct product}\index{direct!product} of \(H\) and \(J\) such that
    \begin{equation}

...
\begin{document}
    ...
    \begin{equation}
        \covariantDerivative{\nu} R^{\mu\nu} = \frac{1}{2} g^{\mu\nu}\covariantDerivative{\nu} R,
    \end{equation}
    which is known as the \defineindex{contracted Binachi identity}, which shows that the Ricci tensor is not, in general, divergence free.
    
    \begin{lma}{Binachi Identities}{}
        The \defineindex{Binachi identity}\footnote{this is sometimes called the second Binachi identity, in which case the first Binachi identity is \(\tensor{R}{^\alpha_{\beta\gamma\delta}} + \tensor{R}{^\alpha_{\gamma\delta\beta}} + \tensor{R}{^\alpha_{\gamma\beta\delta}} = 0\), which is \cref{eqn:first binachi identity}.} is
        \begin{equation}
